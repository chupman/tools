...
def getAllIPs(summary, guest):
    ipmatch = '9.30.'
    ip = [ip.ipAddress for nic in guest.net if nic.ipConfig is not None for ip in nic.ipConfig.ipAddress if ipmatch in ip.ipAddress ]
    if ip:
        print(ip)
        type(ip)
        ip = ip[0]
    elif isinstance(ip, str):
        ip = ip if not None else summary.guest.ipAddress
    else:
...
def vmsummary(summary, guest):
    ...
    ipaddr = getAllIPs(summary, guest)
            ...
            for cluster in clusters: # Iterate through the clusters in the DC
                ...
                for host in hosts: #Iterate through Hosts in the Cluster
                    ...
                    for vm in vms: # Iterate through each VM on the host
                        ...
                        summary = vmsummary(vm.summary, vm.guest) # get vmguest info
